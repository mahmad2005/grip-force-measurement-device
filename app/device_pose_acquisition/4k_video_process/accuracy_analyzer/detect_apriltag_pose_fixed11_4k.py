...
# ╚══════════════════════════════════════════════════════════════════════╝
CIRCLES = [
    (-72.919, 31.297, 4.550, "0"),
    (-17.548, 59.121, 4.550, "A"),
    (-37.819, 31.424, 4.550, "B"),
    (-18.233, 3.689, 4.550, "C"),
    (-57.547, 31.361, 4.550, "D"),
    (-37.441, 58.956, 4.550, "E"),
    (-38.295, 3.606, 4.550, "F"),
    (-57.082, 58.818, 4.550, "G"),
    (-58.197, 3.602, 4.550, "H"),
    (-17.936, 31.591, 4.550, "I"),
    (-1.427, 31.638, 4.550, "J"),
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=6.0,
        help="Extra visual-only offset in tag Y for drawing the cylinder outline. This does NOT affect the reported CYL rel Tag1 values or the 2D top view (default: 0)",
    )
    parser.add_argument(
    ...
    parser.add_argument("--crop-bottom-percent", type=float, default=20,
                        help="Crop this percent from the bottom of the frame (default: 20)")
    parser.add_argument("--crop-left-percent", type=float, default=0.0,
                        help="Crop this percent from the left of the frame (default: 0)")
    parser.add_argument("--crop-right-percent", type=float, default=0.0,
    ...
    if args.circle:
        for i, c in enumerate(args.circle):
            all_circles.append((c[0], c[1], c[2], str(i + len(CIRCLES) + 1)))
    ...
    if not args.quiet and process_stride > 1:
    ...
    # --- 4K CALIBRATION FIX ---
    # The original camera matrix was calibrated for 1080x1920 portrait.
    # Scale it up (or down) to the exact pixel resolution of the current video.
    K = scale_camera_matrix(K_raw, calib_w=1080, calib_h=1920, video_w=width, video_h=height)
        ...
        while True:
            ...
            if interval_sec > 0 and (now - last_progress_print_time >= interval_sec):
                ...
                if total_frames is not None and total_frames > 0:
                    ...
                    print(
                        f"[progress] {frame_idx} frames processed | "
                        f"elapsed {format_seconds_hms(elapsed)} | "
            ...
            process_this_frame = ((frame_idx - 1) % process_stride == 0)
            ...
            if det is not None:
                ...
                if pose_ok:
                    ...
                    # Separate offsets for visual drawing vs geometric mapping.
                    #
                    # cyl_ox/cyl_oy/cyl_oz define the geometric attachment/contact point used
                    # for CYL rel Tag1 and for the 2D top view.
                    #
                    # draw_cyl_* are allowed to differ so the rendered cylinder can be nudged to
                    # visually sit on the white object without moving the minimap footprint.
                    draw_cyl_ox = cyl_ox
                    ...
                    if args.cylinder_axis_sign in {"+", "-"}:
                        ...
                        axis_sign = choose_cylinder_axis_sign(
                            ...
                            K=K_used,
                            dist=dist_used,
                            base_offset_xyz_m=(cyl_ox, cyl_oy, cyl_oz),
                    ...
                    # For table/minimap comparison we want the CYL marker to represent the
                    # cylinder FOOTPRINT / attachment point on the table, not the far tip of
                    # the cylinder. Using the far tip shifts the XY location by roughly the
                    # cylinder length when the axis lies in the table plane (e.g. axis=y).
                    #
                    # So compute the base/contact point separately from the far end.
                    if args.cylinder_mode == "from-base":
                        base_axis_offset = axis_sign * cyl_d
                    else:
                        # centered mode: the base/contact end is half a length opposite the
                        # cylinder extension direction.
                        base_axis_offset = cyl_d - (axis_sign * float(args.cylinder_length_m) / 2.0)

                    if args.cylinder_axis == "x":
                        cyl_contact_tag = np.array([cyl_ox + base_axis_offset, cyl_oy, cyl_oz], dtype=np.float64)
                    elif args.cylinder_axis == "y":
                        cyl_contact_tag = np.array([cyl_ox, cyl_oy + base_axis_offset, cyl_oz], dtype=np.float64)
                    else:
                        cyl_contact_tag = np.array([cyl_ox, cyl_oy, cyl_oz + base_axis_offset], dtype=np.float64)
            ...
            if args.table_tag_ids and table_detector is not None:
                ...
                if freeze_n > 0 and frame_idx >= freeze_n and frozen_table_corners is None and len(table_corners_accum) > 0:
                ...
                if freeze_n > 0 and frame_idx >= freeze_n and frozen_ref_rvec is None and len(ref_rvec_accum) > 0:
                ...
                use_frozen = freeze_n > 0 and frame_idx >= freeze_n and frozen_table_corners is not None
                ...
                if use_frozen:
                    ...
                    tbl_label = f"Table: frozen (avg of first {freeze_n} frames)"
                    ...
                    if freeze_n > 0 and frame_idx <= freeze_n:
            ...
            if all_circles:
                ...
                # Only draw cylinder in minimap if Tag 1 pose is available
                if cyl_contact_cam is not None:
                    if draw_rvec is not None and draw_tvec is not None:
                        # First compute the full 3D point in Tag 1 coordinates using the
                        # same PnP convention on both tags.
                        R1, _ = cv2.Rodrigues(draw_rvec)
                        t1 = draw_tvec.reshape(3)
                        cyl_in_ref = R1.T @ (cyl_contact_cam - t1)
                        cyl_xyz_rel_tag1 = tuple(float(x) for x in cyl_in_ref)
                        cylinder_ref_xyz = (float(cyl_in_ref[0]), float(cyl_in_ref[1]), float(cyl_in_ref[2]))

                        # For minimap / circle comparison, convert the cylinder point to an
                        # image pixel and ray-cast that pixel back onto Tag 1's Z=0 plane.
                        # This guarantees the XY value is directly comparable with the
                        # circle coordinates and with the displayed "Tag 5 on table" value.
                        cyl_img, _ = cv2.projectPoints(
                            cyl_contact_cam.reshape(1, 1, 3),
                        ...
                        table_xy = pixel_to_table_surface(
                            float(cyl_px), float(cyl_py), K_used, draw_rvec, draw_tvec,
                        )
                        if table_xy is not None:
                            cylinder_ref_xy = (float(table_xy[0]), float(table_xy[1]))
                        else:
                            cylinder_ref_xy = (float(cyl_in_ref[0]), float(cyl_in_ref[1]))

                        # Top-left text is now reserved for overlap percentages.
                    else:
                        print("[minimap-warning] Tag 1 pose missing, cannot show cylinder in minimap this frame.")

                # If the current cylinder position is missing or obviously wrong,
                # keep showing the last valid one until a good estimate comes back.
                if is_reasonable_cylinder_position(cylinder_ref_xy, all_circles):
                    last_valid_cylinder_ref_xy = cylinder_ref_xy
                else:
                    cylinder_ref_xy = last_valid_cylinder_ref_xy
                ...
                # Compute top-left overlap percentages using the same 2D table-frame geometry
            ...
            if viewer_export_paths is not None:
                ...
                if "cylinder_ref_xyz" in locals() and cylinder_ref_xyz is not None:
                    # Map Tag1 frame into viewer frame: X->X, Z(height)->Y, Y->Z.
                    raw_viewer_up = abs(float(cylinder_ref_xyz[2]))
                    ...
                    viewer_pos = [
                        float(cylinder_ref_xyz[0]),
                        viewer_up,
                        float(cylinder_ref_xyz[1]),
                    ]
                elif "cylinder_ref_xy" in locals() and cylinder_ref_xy is not None:
